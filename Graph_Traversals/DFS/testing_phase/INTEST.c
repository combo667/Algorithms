...
void Depth_First_Search(graph *g, struct Stack *stk, int start)
{
    ...
    int order[max];
    int count = 0;
    int visit_node, time = 0;
    struct NODE *temp = NULL;

    time_node *tm = NULL;
    tm = (time_node *)malloc(sizeof(time_node) * g->V);
    //printf("%d  ", start);
    tm[start].discovery_time = ++time;
    order[count] = start;
    g->visited[start] = 1;
    Push(stk, start);
    ...
    while (isEmpty(stk) != 1)
    {
        ...
        while (temp != NULL)
        {
            if (g->visited[temp->vertex] == 0)
            {
                ...
                tm[temp->vertex].discovery_time = ++time;
                printf("%d  \n", count);
                order[count++] = temp->vertex;
        ...
        tm[visit_node].finish_time = ++time;
    }
    ...
    printf("\nVisit_Order\tDiscovery_Time\tFinish_Time\n");

    for (int i = 0; i < g->V; i++)
    {
        printf("%d\t\t%d\t\t%d\n", order[i], tm[order[i]].discovery_time, tm[order[i]].finish_time);
    }
    printf("\n");
